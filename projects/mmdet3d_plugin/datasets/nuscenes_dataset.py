...
class CustomNuScenesDataset(NuScenesDataset):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.load_test_label = load_test_label
        if seq_mode:
        ...
        # to constrain the max depth of gt, for evaluation of different depth ranges
        for cls_name, cls_range in self.eval_detection_configs.class_range.items():
            self.eval_detection_configs.class_range[cls_name] = min(cls_range, max_depth)
    ...
    def get_data_info(self, index):
        ...
        if not self.test_mode or self.load_test_label:
