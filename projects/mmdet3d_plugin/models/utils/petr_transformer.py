...
class PETRTransformerDecoder(TransformerLayerSequence):
    ...
    def forward(self, query, *args, **kwargs):
        ...
        for layer in self.layers:
            query, cross_attn = layer(query, *args, **kwargs)
            ...
            cross_attn_weights.append(torch.cat(cross_attn))

        return torch.stack(intermediate), torch.stack(cross_attn_weights)
...
class PETRTemporalTransformer(BaseModule):
    ...
    def forward(self, memory, tgt, query_pos, pos_embed, attn_masks, temp_memory=None, temp_pos=None, mask=None, reg_branch=None):
        ...
        out_dec, cross_attn_weights = self.decoder(
            query=tgt,
            key=memory,
            value=memory,
            key_pos=pos_embed,
        ...
        return  out_dec, memory, cross_attn_weights  # cross_attn_weights: N x B x num_query x (6 * H * W)
        ...
        **kwargs contains some specific arguments of attentions.
        ...
        for layer in self.operation_order:
            if layer == 'self_attn':
                ...
                attn_index += 1
                identity = query
                cross_attn_weights.append(attn_weight)
