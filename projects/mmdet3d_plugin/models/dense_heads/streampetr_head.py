...
class StreamPETRHead(AnchorFreeHead):
    ...
    def position_embeding(self, data, memory_centers, topk_indexes, img_metas):
        ...
        img2lidars = torch.inverse(data['lidar2img'].to('cpu')).to('cuda')
    ...
    def get_transformer_outputs(
        ...
        outs_dec, _, cross_attn_weights = self.transformer(memory, tgt, query_pos, pos_embed, attn_mask, temp_memory, temp_pos)
        ...
        return outs_dec, cross_attn_weights, all_cls_scores, all_bbox_preds
        ...
        **data
        ...
        if attn_mask is None:
            attn_mask = torch.zeros(
                [reference_points.shape[1] + self.num_propagated, reference_points.shape[1] + self.memory_len], 
                device=reference_points.device, dtype=torch.bool
            )
        ...
        outs_dec, cross_attn_weights, all_cls_scores, all_bbox_preds = self.get_transformer_outputs(
            memory, tgt, query_pos, pos_embed, attn_mask, reference_points, temp_memory, temp_pos
        )
